...
- `/lib`: reusable libraries owned by this repository
...
more-specific instructions within those directories. Do not apply a
submodule's conventions to project-owned code, or project-owned conventions to
a submodule.
...
- Add required submodules with `add_subdirectory(modules/<name>)` before
  project-owned libraries and the driver.
...
- Add project-owned libraries from `/lib` after submodules, then add `/src`
  and `/tests`. Link CMake targets by their exported target names rather than
  by raw include or library paths.
